...
# Step 1: Extract mean and standard deviation for Engine_volume
mean_engine <- preprocess_params$mean["Engine_volume"]
std_engine <- preprocess_params$std["Engine_volume"]

# Step 2: Convert standardized Engine_volume values back to original scale
pdp_engine$Engine_volume <- (pdp_engine$Engine_volume * std_engine) + mean_engine

# Step 3: Plot the PDP with original values
plotPartial(pdp_engine, xlab = "Engine Volume", ylab = "Predicted Price_sqrt", main = "Partial Dependence Plot for Engine Volume")
...
# Step 1: Extract mean and standard deviation for Engine_volume
mean_car_age <- preprocess_params$mean["Car_Age"]
std_car_age <- preprocess_params$std["Car_Age"]

# Step 2: Convert standardized Engine_volume values back to original scale
pdp_Car_Age$Car_Age <- (pdp_Car_Age$Car_Age * std_car_age) + mean_car_age

# Step 3: Plot the PDP with original values
plotPartial(pdp_Car_Age, xlab = "Car_Age", ylab = "Predicted Price_sqrt", main = "Partial Dependence Plot for Car Age")
...
# Step 1: Extract mean and standard deviation for Engine_volume
mean_mileage <- preprocess_params$mean["Mileage_sqrt"]
std_mileage <- preprocess_params$std["Mileage_sqrt"]

# Step 2: Convert standardized Engine_volume values back to original scale
pdp_mileage$Mileage_sqrt <- (pdp_mileage$Mileage_sqrt * std_mileage) + mean_mileage


# Plot the PDP
plotPartial(pdp_mileage, xlab = "Mileage_sqrt", ylab = "Predicted Price_sqrt", main = "Partial Dependence Plot for Mileage Square Root")
